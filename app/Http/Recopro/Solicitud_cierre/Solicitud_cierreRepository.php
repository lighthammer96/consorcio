...
class Solicitud_cierreRepository implements Solicitud_cierreInterface
{
    protected $model;
    private static $_ACTIVE = 'A';
    public function __construct(Solicitud_cierre $model)
    {
        $this->model = $model;
    }
    ...
    {
        // $array = 
        return $this->model->where("periodo", $periodo)->where(function ($q) use ($s) {
...
// >>>>>>> 4870a79d0dc412d8e5cb96175d98abd570e4358d
    ...
    public function get_solicitud_articulo($cCodConsecutivo, $nConsecutivo)
    {
        ...
        FROM ERP_Solicitud_cierreArticulo AS sa
    ...
    public function get_solicitud_articulo_vehiculo($cCodConsecutivo, $nConsecutivo)
    {
        ...
        FROM ERP_Solicitud_cierreArticulo AS sa
        INNER JOIN ERP_Productos AS p ON(sa.idarticulo=p.id)
        INNER JOIN ERP_Solicitud_cierreDetalle AS sd ON(sd.cCodConsecutivo=sa.cCodConsecutivo AND sd.nConsecutivo=sa.nConsecutivo AND sd.idarticulo=sa.idarticulo)
    ...
    public function get_solicitud_detalle($cCodConsecutivo, $nConsecutivo)
    {
        ...
        $sql = "SELECT * FROM ERP_Solicitud_cierreDetalle WHERE cCodConsecutivo='{$cCodConsecutivo}' AND nConsecutivo={$nConsecutivo}";
        $result = DB::select($sql);
    ...
    public function get_solicitud_detalle_series($cCodConsecutivo, $nConsecutivo, $id_solicitud_articulo)
    {
        ...
        $sql = "SELECT * FROM ERP_Solicitud_cierreDetalle WHERE cCodConsecutivo='{$cCodConsecutivo}' AND nConsecutivo={$nConsecutivo} AND id_solicitud_articulo={$id_solicitud_articulo}";
        $result = DB::select($sql);
       ...
       public function get_solicitud_credito($cCodConsecutivo, $nConsecutivo)
    {
        ...
        FROM ERP_Solicitud_cierreCredito  AS s
    ...
    public function get_solicitud_cronograma($cCodConsecutivo, $nConsecutivo)
    {
        ...
        CASE WHEN sc.saldo_cuota=0 THEN 'PAGADO' ELSE 'PENDIENTE' END AS estado, FORMAT(sc.fecha_vencimiento, 'yyyy-MM-dd') AS fecha_vencimiento_credito
         FROM ERP_Solicitud_cierreCronograma AS sc
    ...
    public function mostrar_aprobaciones($cCodConsecutivo, $nConsecutivo) {
        ...
        FROM ERP_Solicitud_cierreConformidad AS sc
    ...
    public function update_saldos_solicitud($data) {
        $sql_update = "UPDATE ERP_Solicitud_cierre SET 
    ...
    public function update_montos_mora($data) {
        $sql_update = "UPDATE ERP_Solicitud_cierreCronograma SET 
    ...
    public function validar_serie($idserie) {
        $sql = "SELECT * FROM ERP_Solicitud_cierre AS s
        INNER JOIN ERP_Solicitud_cierreArticulo AS sa ON(sa.cCodConsecutivo=s.cCodConsecutivo AND sa.nConsecutivo=s.nConsecutivo)
        INNER JOIN ERP_Solicitud_cierreDetalle AS sd ON(sa.id=sd.id_solicitud_articulo AND sa.cCodConsecutivo=sd.cCodConsecutivo AND sa.nConsecutivo=sd.nConsecutivo)
    ...
    public function copiar_solicitud($cCodConsecutivo, $nConsecutivo) {
        ...
        SET NOCOUNT ON; EXEC	@return_value = [dbo].[VTA_CopiarSolicitud_cierreVenta]
    ...
    public function obtener_separaciones($cCodConsecutivo, $nConsecutivo) {
        ...
        FROM dbo.ERP_Solicitud_cierreSeparacion AS ss 
    ...
    public function obtener_series($id_solicitud_articulo) {
        $sql = "SELECT * FROM ERP_Solicitud_cierreDetalle  AS sd 
