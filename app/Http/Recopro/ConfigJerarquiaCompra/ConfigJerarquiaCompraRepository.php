...
class ConfigJerarquiaCompraRepository implements ConfigJerarquiaCompraInterface
{
    ...
    {
        return $this->model->where('nIdTienda', $attributes['nIdTienda'])
            ->where('nIdMoneda', $attributes['nIdMoneda'])
            ->where('montoInicio', '<=', $attributes['total'])
            ->where('montoFin', '>=', $attributes['total'])
            ->where('dFecIni', '<=', $attributes['date'])
            ->where('dFecFin', '>=', $attributes['date'])
            ->get();
    }
