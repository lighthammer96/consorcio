...
//        $this->middleware('json');
    ...
    public function all(Request $request, Query_stockInterface $repo)
    {
        ...
        $params = ['tipoCompraVenta', 'code_article', 'id', 'Articulo', 'Categoria', 'Unidad', 'Almacen', 'Localizacion', 'Lote', 'Serie', 'Disponible', 'Remitido', 'Total', 'Transito', 'Costo_Promedio_Unitario', 'Costo_Total', 'Chasis', 'Motor', 'Color', 'Ano', 'Ingreso'];
        // print_R($filtro_art);
    ...
    public function excel(Query_stockInterface $repo, Request $request)
    {
        ...
        $filtro_idLoc = $request->input('filtro_idLoc');
        $filtro_cate = $request->input('filtro_cate');
        return generateExcel($this->generateDataExcel($repo->allFiltro($s, $filtro_art, $filtro_idAlm, $filtro_idLoc,
            $filtro_cate)), 'LISTA DE ARTICULOS CON STOCK', 'Articulo');
    }
