...
class ProveedorController extends Controller
{
    ...
    public function all(Request $request, ProveedorInterface $repo)
    {
        try {
            $filter = $request->all();
            $params = ['id', 'tipodoc', 'documento', 'razonsocial', 'contacto', 'direccion', 'correo_electronico',
                'celular', 'ubigeo', 'id_tipoProveedor', 'IdTipoDocumento'];
            ...
            foreach ($data as $d) {
                $type_doc = $d->tipo_u;
                $d->type_doc = ($type_doc) ? $type_doc->cDescripcion : '';
                $type_prov = $d->tipo_c;
                $d->type_prov = ($type_prov) ? $type_prov->descripcion : '';
                $type_doc_v = $d->tipo_doc_vent;
                $d->type_doc_v = ($type_doc_v) ? $type_doc_v->Descripcion : '';
                $location = $d->ubigeo_u;
                $d->district = ($location) ? $location->cDistrito : '';
                unset($d->tipo_u, $d->tipo_c, $d->tipo_doc_vent, $d->ubigeo_u);
            }
