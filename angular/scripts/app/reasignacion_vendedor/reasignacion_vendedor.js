...
(function () {
    'use strict';
    angular.module('sys.app.reasignacion_vendedor')
        .config(Config)
        .controller('ReasignacionVendedorCtrl', ReasignacionVendedorCtrl);

    Config.$inject = ['$stateProvider', '$urlRouterProvider'];
    ReasignacionVendedorCtrl.$inject = ['$scope', '_', 'RESTService', 'AlertFactory', 'Helpers'];

    function ReasignacionVendedorCtrl($scope, _, RESTService, AlertFactory, Helpers) {
        ...
        function obtener_data_for_solicitud() {
            RESTService.all('reasignacion_vendedor/data_form', '', function (response) {
        ...
        btn_editar_cliente.click(function (e) {
            if (cliente_id_or.val() == '') {
                ...
                RESTService.get('reasignacion_vendedor/find', id, function (response) {
                    if (!_.isUndefined(response.status) && response.status) {
        ...
        function saveCliente() {
            ...
            if (bval) {
                ...
                RESTService.updated('reasignacion_vendedor/createCliente', cli_id, params, function (response) {
                    if (!_.isUndefined(response.status) && response.status) {
        ...
        function getDataFormCustomer() {
            RESTService.all('reasignacion_vendedor/data_form_customer', '', function (response) {
                if (!_.isUndefined(response.status) && response.status) {
        ...
        function getCliente() {
            ...
            if (bval) {
                ...
                RESTService.get('reasignacion_vendedor/get_cliente', id, function (response) {
                    if (!_.isUndefined(response.status) && response.status) {
        ...
        function getDepartamentoPersona(bandera) {
            var id = "0";
            RESTService.get('reasignacion_vendedor/TraerDepartamentos', id, function (response) {
                if (!_.isUndefined(response.status) && response.status) {
                    var data_p = response.data;
                    cRegion.html('');
        ...
        function getProvinciaPersona(bandera, id) {
            RESTService.get('reasignacion_vendedor/TraerProvincias', id, function (response) {
                if (!_.isUndefined(response.status) && response.status) {
                    var data_p = response.data;
                    console.log(data_p);
                    cProvincia.html('');
        ...
        function getDistritoPersona(bandera, id) {
            RESTService.get('reasignacion_vendedor/TraerDistritos', id, function (response) {
                if (!_.isUndefined(response.status) && response.status) {
                    var data_p = response.data;
                    console.log(data_p);
                    cUbigeo.html('');
        ...
        RESTService.get('reasignacion_vendedor/traerSectorOrd', id, function(response) {
                 if (!_.isUndefined(response.status) && response.status) {
                     var data_p = response.data;
                     console.log(data_p);
                      idsector.html('');
        ...
        documento.keypress(function (e) {
            ...
            if (code == 13) {
                ...
                RESTService.get('reasignacion_vendedor/get_cliente_persona', documentoEnvio, function (response) {
                    if (!_.isUndefined(response.status) && response.status) {
        ...
        function cargartableMovAr() {
            ...
            table_container_cc2.jtable({
                ...
                cache: false,
                actions: {
                    listAction: base_url + '/reasignacion_vendedor/getArticulosSelect'
                },
                toolbar: {
                    items: [{
                        cssClass: 'buscador',
        ...
        function addArtNada() {
            ...
            if (bval) {
                ...
                // alert(id);
                RESTService.get('reasignacion_vendedor/get_precios_list', id, function (response) {
                    if (!_.isUndefined(response.status) && response.status) {
        ...
        $scope.addSeleccSerie = function () {
            ...
            if (bval) {
                ...
                RESTService.get('reasignacion_vendedor/validateCantSerie', id, function (response) {
                    if (!_.isUndefined(response.status) && response.status) {
                        if (response.data == 'N') {
                            AlertFactory.textType({
                                title: '',
                ...
                actions: {
                    listAction: function (postData, jtParams) {
                        return $.Deferred(function ($dfd) {
                            $.ajax({
                                url: base_url + '/reasignacion_vendedor/' + url,
                                type: 'POST',
                                dataType: 'json',
                ...
                recordsLoaded: function (event, data) {
                    ...
                    $('.valcheck').on('ifChanged', function(event){
                        ...
                        if ($(this).prop('checked')) {
                            $.post("reasignacion_vendedor/validar_serie", { idserie: idserie},
        ...
        function addArticuloTable(idProducto, desProducto, cantProducto, ver, codigo, tipo, codl, datl, idAlmacen, idLocalizacion, costo, costo_total, precio, presio_total, impuesto_articulo, lote_articulo, cOperGrat, iddescuento, posee_serie, um_id, series_id_sd, articulos_id_sd, type_id, idCategoria, descripcion_articulo, id_solicitud_articulo) {
            ...
            // boton ver series
            var button_ver_series = "";
            if(series_id_sd != "" && articulos_id_sd != "") {
                button_ver_series = '&nbsp;&nbsp;<button  class="btn btn-primary btn-xs ver-series" data-id_solicitud_articulo="' + id_solicitud_articulo + '" title="Ver Series" type="button"><span class="fa fa-search"></span>&nbsp;Ver Series</button>';
            }
            ...
            var btn3 = $('<center><button ' + desactivar_del + ' class="btn btn-danger btn-xs delMovPro" data-tipo="' + tipo + '" title="Eliminar" data-id="' + codigo + '" type="button"><span class="fa fa-trash"></span></button>' + button_series + button_ver_series + '</center>');
            // td6.append(btn1);
        ...
        $(document).on("blur", "input[name='precio_unitario[]']", function () {
            ...
            $.post("reasignacion_vendedor/validar_parametro_categoria", {},
                function (data, textStatus, jqXHR) {
                    // console.log(data);
                    if(data.length > 0) {
        ...
        $(document).on("click", ".ver-series", function () {
            var id_solicitud_articulo = $(this).data("id_solicitud_articulo");
            // alert(id_solicitud_articulo);
            $.post("reasignacion_vendedor/obtener_series", { id_solicitud_articulo: id_solicitud_articulo },
            function (data, textStatus, jqXHR) {
                if (data.length > 0) {
                    var html = '';
                    $("#detalle-series").html("");
                    for (var i = 0; i < data.length; i++) {
                        ...
                        html += '   <td>' + data[i].nombreSerie + '</td>';
                        html += '   <td>' + data[i].chasis + '</td>';
                        html += '   <td>' + data[i].motor + '</td>';
                        html += '   <td>' + data[i].anio_fabricacion + '</td>';
                        html += '   <td>' + data[i].anio_modelo + '</td>';
                        html += '   <td>' + data[i].color + '</td>';
                        html += '</tr>';
                    }
                    $("#detalle-series").html(html);
        ...
        function getLocaStock(idl, ident, idPrAl, idLocalizacion) {
            ...
            var id = idl;
            // alert(id);
            RESTService.get('reasignacion_vendedor/getLocaStock', id, function (response) {
                if (!_.isUndefined(response.status) && response.status) {
        ...
        $(document).on("keyup", "#nro_cuotas", function () {
            ...
            $.post("reasignacion_vendedor/factor_credito", { nro_cuotas: nro_cuotas },
                function (data, textStatus, jqXHR) {
                ...
                $.post("reasignacion_vendedor/guardar_solicitud", $("#formulario-solicitud").serialize() + "&" + $("#formulario-creditos").serialize() + "&cCodConsecutivo=" + cCodConsecutivo.val() + "&tipo_solicitud=" + $("#tipo_solicitud").val() + "&idmoneda=" + $("#IdMoneda").val() + "&id_tipoDoc_Venta_or_solicitud=" + $("#id_tipoDoc_Venta_or_solicitud").val() + "&idvendedor=" + $("#idvendedor").val() + "&descuento_id=" + $("#totalDescuento").val()+ "&estado=" + $("#estado").val(),
                    function (data, textStatus, jqXHR) {
                        ...
                        if (data.status == "i" || data.status == "m") {
                            ...
                            _.each(data.separaciones, function (item) {
        ...
        $scope.mostrar_aprobaciones = function () {
            ...
            if (bval) {
                $.post("reasignacion_vendedor/mostrar_aprobaciones", { cCodConsecutivo: cCodConsecutivo.val(), nConsecutivo: nConsecutivo.val() },
                    function (data, textStatus, jqXHR) {
                        if (data.length > 0) {
                            var html = '';
        ...
        $scope.imprimir_solicitud = function () {
            ...
            window.open("reasignacion_vendedor/imprimir_solicitud/" + id);
        ...
        $scope.imprimir_clausula_solicitud = function () {
            ...
            window.open("reasignacion_vendedor/imprimir_clausula_solicitud/" + id);
        ...
        $scope.imprimir_cronograma = function () {
            ...
            var id = cCodConsecutivo + "|" + nConsecutivo;
            window.open("reasignacion_vendedor/imprimir_cronograma/" + id);
        ...
        $scope.cancelar_solicitud = function () {
            ...
            $.post("reasignacion_vendedor/anular_solicitud", { cCodConsecutivo: cCodConsecutivo, nConsecutivo: nConsecutivo},
                function (data, textStatus, jqXHR) {
        ...
        $scope.agregar_separaciones = function () {
            ...
            $.post("reasignacion_vendedor/obtener_separaciones", { cCodConsecutivo: cCodConsecutivo.val(), nConsecutivo: nConsecutivo.val() },
                function (data, textStatus, jqXHR) {
                    ...
                    if(data.length > 0) {
                        ...
                        $("#detalle-separaciones").html(html);
                    } else {
        ...
        $scope.guardar_separaciones = function () {
            ...
            $.post("reasignacion_vendedor/guardar_separaciones", $("#formulario-separaciones").serialize()+"&cCodConsecutivo="+cCodConsecutivo.val()+"&nConsecutivo="+nConsecutivo.val(),
                function (data, textStatus, jqXHR) {
                    if (data.status == "i") {
        ...
        $scope.enviar_solicitud = function () {
            ...
            if (bval) {
                ...
                // alert($("#estado").val());
                $.post("reasignacion_vendedor/enviar_solicitud", $("#formulario-solicitud").serialize() + "&estado=" + $("#estado").val(),
                    function (data, textStatus, jqXHR) {
                        if (data.status == "i") {
                            LoadRecordsButtonSolicitud.click();
        ...
        function find_solicitud(id) {
            ...
            $.post("reasignacion_vendedor/find", { id: id },
                function (data, textStatus, jqXHR) {
                    // console.log(data);
        ...
        table_container_solicitud.jtable({
            ...
            sorting: true,
            actions: {
                listAction: base_url + '/reasignacion_vendedor/list',
                // createAction: base_url + '/reasignacion_vendedor/create',
                // updateAction: base_url + '/reasignacion_vendedor/update',
                // deleteAction: base_url + '/reasignacion_vendedor/delete',
            },
            messages: {
            ...
            toolbar: {
                ...
                //     click: function () {
                //         $scope.openDoc('reasignacion_vendedor/excel', {});
                //     }
                // }, {
            ...
            recordsLoaded: function (event, data) {
                ...
                $('.eliminar-solicitud').click(function (e) {
                    ...
                    $.post("reasignacion_vendedor/eliminar_solicitud", { cCodConsecutivo: ccodconsecutivo, nConsecutivo: nconsecutivo },
                        function (data, textStatus, jqXHR) {
                            console.log(data);
                            if (data.status == "e") {
                                LoadRecordsButtonSolicitud.click();
    ...
    function Config($stateProvider, $urlRouterProvider) {
        $stateProvider
            ...
            .state('reasignacion_vendedor', {
                url: '/reasignacion_vendedor',
                templateUrl: base_url + '/templates/reasignacion_vendedor/base.html',
                controller: 'ReasignacionVendedorCtrl'
            });
